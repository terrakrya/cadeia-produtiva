<template>
  <div>
    ...
    <p class="text-muted small mb-3">
      <b-icon-info-circle /> 
      Gerencie as regiões onde esta espécie é encontrada. Cada região pode conter vários municípios.
    </p>
    ...
    <div v-if="regions.length > 0">
      ...
      >
        <template #cell(actions)="data">
          <b-button @click="showEditModal(data.item)" variant="warning" size="sm">
            Editar
          </b-button>
    ...
    <b-modal v-model="isModalVisible" :title="modalTitle" size="lg" hide-footer>
      <b-form @submit.prevent="saveRegion">
        ...
        <div v-for="(municipality, index) in form.municipalities" :key="index" class="border rounded p-3 mb-3">
                ...
                >
                  <template #first>
                    <b-form-select-option :value="null" disabled>
                      {{ municipality.uf ? '-- Selecione --' : '-- Selecione o Estado primeiro --' }}
        ...
        <div class="text-right">
            ...
            <b-button type="submit" variant="primary" class="ml-2" :disabled="isSaving">
    ...
    <b-modal
      ...
      @ok="deleteRegion"
    >
      <p>Tem certeza que deseja excluir a região "{{ regionToDelete?.name }}"?</p>
...
export default {
  ...
  methods: {
    ...
    async fetchRegions() {
      this.isLoading = true
      try {
        const response = await this.$axios.get(`/regions?specie=${this.specieId}`)
    ...
    addMunicipality() {
      this.form.municipalities.push({ name: '', state: '', uf: null, codigo_uf: null })
    },
    ...
    async saveRegion() {
      ...
      try {
        ...
        this.notify(`Região ${this.isEditing ? 'atualizada' : 'salva'} com sucesso!`)
        await this.fetchRegions()
        this.hideModal()
      } catch (error) {
